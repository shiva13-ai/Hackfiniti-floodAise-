import { sensorReadingSchema, analysisSchema, deviceRegistrationSchema } from "../shared/schema.js";
...
function computeAdjustedRisk(satelliteRisk, latestWaterLevel) {
    if (latestWaterLevel === null)
        return { adjustedRisk: satelliteRisk, delta: 0, confidence: "satellite-only" };
    if (latestWaterLevel > CRITICAL_WATER_THRESHOLD) {
        const adjusted = Math.min(1, Math.max(satelliteRisk, 0.85));
        return { adjustedRisk: adjusted, delta: +(adjusted - satelliteRisk).toFixed(3), confidence: "iot-validated-critical" };
    }
    if (latestWaterLevel > MODERATE_WATER_THRESHOLD) {
        const adjusted = Math.min(1, satelliteRisk + ALPHA);
        return { adjustedRisk: adjusted, delta: +(adjusted - satelliteRisk).toFixed(3), confidence: "iot-validated" };
    }
    return { adjustedRisk: satelliteRisk, delta: 0, confidence: "iot-confirmed-stable" };
}
// ---- Sensor Reliability Score ----
function computeReliability(sensorId) {
...
export function registerRoutes(app) {
    ...
    app.post("/api/analyses", (req, res) => {
        ...
        const analysis = { ...result.data, id, createdAt: new Date().toISOString() };
        analyses.set(id, analysis);
        res.status(201).json(analysis);
    });
    app.delete("/api/analyses/:id", (req, res) => {
        if (!analyses.delete(req.params.id))
            return res.status(404).json({ error: "Not found" });
        res.status(204).send();
    });
    // ---- Device Registration ----
    ...
    app.post("/api/devices", (req, res) => {
        ...
        if (existing)
            return res.status(409).json({ error: "Sensor ID already registered" });
        const id = `dev-${++deviceCounter}`;
    ...
    app.delete("/api/devices/:id", (req, res) => {
        ...
        if (!device.isExternal)
            return res.status(403).json({ error: "Cannot delete built-in devices" });
    ...
    app.post("/api/sensors/data", (req, res) => {
        ...
        if (!sensors.has(sensorId))
            sensors.set(sensorId, { sensorId, readings: [], rejectedCount: 0, totalCount: 0 });
        const reading = {
        ...
        if (s.readings.length > 1000)
            s.readings = s.readings.slice(-1000);
